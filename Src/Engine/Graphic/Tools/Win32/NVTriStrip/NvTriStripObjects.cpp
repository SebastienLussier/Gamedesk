									 ...
									 const unsigned short maxIndex)
{
	...
	{	
		...
		//  exist in the list
		NvFaceInfo *faceInfo = GD_NEW(NvFaceInfo, 0, "NvTriStrip")(v0, v1, v2);
		...
		if (edgeInfo01 == NULL)
		{
			...
			// create the info
			edgeInfo01 = GD_NEW(NvEdgeInfo, 0, "NvTriStrip")(v0, v1);
		...
		if (edgeInfo12 == NULL)
		{
			...
			// create the info
			edgeInfo12 = GD_NEW(NvEdgeInfo, 0, "NvTriStrip")(v1, v2);
		...
		if (edgeInfo20 == NULL)
		{
			...
			// create the info
			edgeInfo20 = GD_NEW(NvEdgeInfo, 0, "NvTriStrip")(v2, v0);
	...
	while (nextFace != NULL && !IsMarked(nextFace))
	{
		...
		if( (nextNextFace == NULL) || (IsMarked(nextNextFace)) )
		{
			...
			if( ((testNextFace != NULL) && !IsMarked(testNextFace)) )
			{
				...
				NvFaceInfo* tempFace = GD_NEW(NvFaceInfo, 0, "NvTriStrip")(nv0, nv1, nv0);
...
void NvStripifier::RemoveSmallStrips(NvStripInfoVec& allStrips, NvStripInfoVec& allBigStrips, NvFaceInfoVec& faceList)
{
	...
	bool *bVisitedList = GD_NEW_ARRAY(bool, tempFaceList.size(), 0, "NvTriStrip");
	memset(bVisitedList, 0, tempFaceList.size()*sizeof(bool));
	
	VertexCache* vcache = GD_NEW(VertexCache, 0, "NvTriStrip")(cacheSize);
...
bool NvStripifier::FaceContainsIndex(const NvFaceInfo& face, const size_t index)
{
                                            ...
                                            NvEdgeInfoVec& edgeInfos, NvFaceInfoVec& outFaceList)
{
	...
	for(size_t i = 0; i < allStrips.size(); i++)
	{
		...
		if(actualStripSize /*allStrips[i]->m_faces.size()*/ > threshold)
		{
			...
			{
				currentStrip = GD_NEW(NvStripInfo, this, "NvTriStrip")(startInfo, 0, -1);
...
//    outStrips.push_back(tempStrips[i]);
	...
	if(tempStrips2.size() != 0)
	{
		...
		VertexCache* vcache = GD_NEW(VertexCache, this, "NvTriStrip")(cacheSize);
...
void NvStripifier::FindAllStrips(NvStripInfoVec &allStrips,
	...
	while (!done)
	{
		...
		//
		NvStripInfoVec *experiments = GD_NEW_ARRAY(NvStripInfoVec, numSamples * 6, this, "NvTriStrip");
		...
		for (int i = 0; i < numSamples; i++)
		{
			...
			GD_ASSERT(nextFace->m_stripId < 0);
			
			// build the strip off of this face's 0-1 edge
			NvEdgeInfo *edge01 = FindEdgeInfo(allEdgeInfos, nextFace->m_v0, nextFace->m_v1);
			NvStripInfo *strip01 = GD_NEW(NvStripInfo, 0, "NvTriStrip")(NvStripStartInfo(nextFace, edge01, true), stripId++, experimentId++);
			...
			NvStripInfo *strip10 = GD_NEW(NvStripInfo, 0, "NvTriStrip")(NvStripStartInfo(nextFace, edge10, false), stripId++, experimentId++);
			...
			NvStripInfo *strip12 = GD_NEW(NvStripInfo, 0, "NvTriStrip")(NvStripStartInfo(nextFace, edge12, true), stripId++, experimentId++);
			...
			NvStripInfo *strip21 = GD_NEW(NvStripInfo, 0, "NvTriStrip")(NvStripStartInfo(nextFace, edge21, false), stripId++, experimentId++);
			...
			NvStripInfo *strip20 = GD_NEW(NvStripInfo, 0, "NvTriStrip")(NvStripStartInfo(nextFace, edge20, true), stripId++, experimentId++);
			...
			NvStripInfo *strip02 = GD_NEW(NvStripInfo, 0, "NvTriStrip")(NvStripStartInfo(nextFace, edge02, false), stripId++, experimentId++);
		...
		for (int i = 0; i < numExperiments; i++){
			...
			while (FindTraversal(allFaceInfos, allEdgeInfos, stripIter, startInfo)){
				...
				stripIter = GD_NEW(NvStripInfo, 0, "NvTriStrip")(startInfo, stripId++, experimentId);
		...
		for (int i = 0; i < numExperiments; i++)
		{
			if (i != bestIndex)
			{
				...
				{
					GD_DELETE(experiments[i][j]);
				}
			}
		}
		
		// delete the array that we used for all experiments
		GD_DELETE_ARRAY(experiments);
  }
}
