					...
					PrimitiveGroup** primGroups, unsigned short* numGroups)
{
	...
	if(bListsOnly)
	{
		...
		*numGroups = 1;
        (*primGroups) = GD_NEW_ARRAY(PrimitiveGroup, *numGroups, 0, "NvTriStrip");
        ...
        primGroupArray[0].indices    = GD_NEW_ARRAY(unsigned short, numIndices, 0, "NvTriStrip");
	...
	else
	{
		...
		GD_ASSERT( (bStitchStrips && (numSeparateStrips == 1)) || !bStitchStrips);
		...
		*numGroups = numSeparateStrips; //for the strips
		...
		for(size_t stripCtr = 0; stripCtr < numSeparateStrips; stripCtr++)
		{
			...
			primGroupArray[stripCtr].indices    = GD_NEW_ARRAY(unsigned short, stripLength, 0, "NvTriStrip");
		...
		if(tempFaces.size() != 0)
		{
			...
			primGroupArray[faceGroupLoc].indices    = GD_NEW_ARRAY(unsigned short, tempFaces.size() * 3, 0, "NvTriStrip");
	...
	for(size_t i = 0; i < tempStrips.size(); i++)
	{
		for(size_t j = 0; j < tempStrips[i]->m_faces.size(); j++)
		{
			GD_DELETE(tempStrips[i]->m_faces[j]);
			tempStrips[i]->m_faces[j] = NULL;
		}
		GD_DELETE(tempStrips[i]);
		tempStrips[i] = NULL;
	}
...
{
	(*remappedGroups) = GD_NEW_ARRAY(PrimitiveGroup, numGroups, 0, "NvTriStrip");

	//caches oldIndex --> newIndex conversion
	int *indexCache;
	indexCache = GD_NEW_ARRAY(int, numVerts, 0, "NvTriStrip");
	...
	for(int i = 0; i < numGroups; i++)
	{
		...
		(*remappedGroups)[i].indices    = GD_NEW_ARRAY(unsigned short, numIndices, 0, "NvTriStrip");
